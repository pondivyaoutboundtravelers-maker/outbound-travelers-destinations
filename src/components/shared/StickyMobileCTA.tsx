'use client'

import React, { useEffect, useState } from 'react'

interface StickyMobileCTAProps {
  whatsappUrl: string
  phoneNumber: string
  accentColor: string
  onEnquire: () => void
}

export default function StickyMobileCTA({ whatsappUrl, phoneNumber, accentColor, onEnquire }: StickyMobileCTAProps) {
  const [isVisible, setIsVisible] = useState(false)
  const [isSubmitted, setIsSubmitted] = useState(false)

  useEffect(() => {
    const handleScroll = () => {
      const scrollHeight = document.documentElement.scrollHeight - window.innerHeight
      if (scrollHeight <= 0) return
      
      const scrollPercentage = (window.scrollY / scrollHeight) * 100
      if (scrollPercentage > 20 && !isSubmitted) {
        setIsVisible(true)
      } else {
        setIsVisible(false)
      }
    }

    const handleSuccess = () => {
      setIsSubmitted(true)
      setIsVisible(false)
    }

    window.addEventListener('scroll', handleScroll)
    window.addEventListener('formSubmitSuccess', handleSuccess)

    return () => {
      window.removeEventListener('scroll', handleScroll)
      window.removeEventListener('formSubmitSuccess', handleSuccess)
    }
  }, [isSubmitted])

  if (!isVisible) return null

  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 h-[64px] bg-white border-t border-border-soft flex items-center z-50 shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
      {/* WhatsApp Section */}
      <a
        href={whatsappUrl}
        target="_blank"
        rel="noopener noreferrer"
        className="flex-1 flex flex-col items-center justify-center h-full text-green-600 border-r border-border-soft hover:bg-gray-50 transition-colors"
        aria-label="WhatsApp Kashmir Team"
      >
        <svg className="w-6 h-6 fill-current" viewBox="0 0 24 24">
          <path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946C.06 5.348 5.397.01 12.008.01c3.202.001 6.212 1.246 8.477 3.514 2.266 2.268 3.507 5.28 3.505 8.484-.004 6.657-5.34 11.997-11.953 11.997-2.005-.001-3.973-.502-5.724-1.455L0 24zm6.59-4.846c1.6.95 3.197 1.451 4.805 1.452 5.426.002 9.84-4.408 9.843-9.84.002-2.63-1.023-5.101-2.883-6.963C16.552 1.95 14.09 .928 11.46.928c-5.43 0-9.844 4.412-9.847 9.844-.001 1.733.456 3.424 1.323 4.908l-.893 3.256 3.32-.871c1.554.848 3.033 1.22 4.604 1.22zm10.777-7.44c-.26-.13-1.536-.759-1.774-.846-.237-.087-.41-.13-.58.13-.172.26-.666.846-.817 1.018-.152.173-.303.195-.563.065-.26-.13-1.097-.404-2.09-1.289-.773-.69-1.295-1.542-1.447-1.802-.152-.26-.016-.4.113-.53.117-.117.26-.303.39-.455.13-.152.173-.26.26-.433.087-.173.043-.325-.022-.455-.065-.13-.58-1.402-.796-1.921-.21-.507-.442-.437-.607-.446-.157-.008-.337-.01-.518-.01-.18 0-.476.068-.724.338-.25.27-.95.93-.95 2.27 0 1.34.975 2.632 1.111 2.816.136.183 1.92 2.925 4.65 4.103.65.28 1.157.447 1.553.573.654.208 1.248.179 1.718.109.523-.078 1.536-.628 1.753-1.235.217-.607.217-1.127.152-1.235-.065-.108-.238-.172-.498-.302z" />
        </svg>
        <span className="text-[10px] font-bold mt-1 uppercase">WhatsApp</span>
      </a>

      {/* Call Section */}
      <a
        href={`tel:${phoneNumber}`}
        className="flex-1 flex flex-col items-center justify-center h-full text-brand-navy border-r border-border-soft hover:bg-gray-50 transition-colors"
        aria-label="Call Outbound Travelers"
      >
        <svg className="w-6 h-6 fill-none stroke-current" viewBox="0 0 24 24" strokeWidth="2">
          <path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.94.725l.548 2.2a1 1 0 01-.321.988l-1.305.98a10.582 10.582 0 004.872 4.872l.98-1.305a1 1 0 01.988-.321l2.2.548a1 1 0 01.725.94V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
        </svg>
        <span className="text-[10px] font-bold mt-1 uppercase">Call</span>
      </a>

      {/* Enquire Section */}
      <button
        onClick={onEnquire}
        className="flex-[2] flex items-center justify-center h-full text-white font-bold text-sm uppercase px-4 transition-colors"
        style={{ backgroundColor: accentColor }}
      >
        Enquire Now
      </button>
    </div>
  )
}
