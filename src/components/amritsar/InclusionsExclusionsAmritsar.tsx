...
export default function InclusionsExclusionsAmritsar() {
  ...
  return (
    <section className="py-16 px-4 md:px-8 max-w-5xl mx-auto scroll-mt-20">
      ...
      <div className="hidden md:grid grid-cols-2 gap-8">
        ...
        <div className="bg-white border border-border-soft rounded-2xl p-6 md:p-8 shadow-sm">
          ...
          <ul className="space-y-3.5">
            {amritsarData.inclusions.map((item, idx) => (
              <li key={idx} className="flex items-start gap-3">
                ...
                <span className="text-sm md:text-base text-muted flex-1 leading-normal font-medium">
                  {item}
                  {item.includes('Wagah Border transport') && (
                    <span className="inline-block ml-2 bg-[#FEF3C7] text-[#A16207] text-[9px] font-bold px-2 py-0.5 rounded border border-[#A16207]/30 uppercase tracking-wider">
        ...
        <div className="bg-white border border-border-soft rounded-2xl p-6 md:p-8 shadow-sm">
          ...
          <ul className="space-y-3.5">
            {amritsarData.exclusions.map((item, idx) => (
              <li key={idx} className="flex items-start gap-3">
                ...
                <span className="text-sm md:text-base text-muted leading-normal font-medium">
                  {item}
                </span>
              </li>
            ))}
              ...
              {amritsarData.inclusions.map((item, idx) => (
                <li key={idx} className="flex items-start gap-2.5">
                  ...
                  <span className="text-xs text-muted flex-1 leading-normal font-medium">
                    {item}
                    {item.includes('Wagah Border transport') && (
                      <span className="inline-block ml-1.5 bg-[#FEF3C7] text-[#A16207] text-[8px] font-bold px-1.5 py-0.5 rounded border border-[#A16207]/30">
                        Included
                      </span>
                    )}
                  </span>
      ...
      <div className="text-center mt-10">
        ...
        >
          Request Custom Amritsar Package
        </button>
      </div>
    </section>
  )
