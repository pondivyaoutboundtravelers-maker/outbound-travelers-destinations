...
// Kashmir Components
import KashmirHero from '@/components/kashmir/KashmirHero'
import QuickSummaryStrip from '@/components/kashmir/QuickSummaryStrip'
import WhyKashmir from '@/components/kashmir/WhyKashmir'
import TopExperiences from '@/components/kashmir/TopExperiences'
import HoneymoonSpecial from '@/components/kashmir/HoneymoonSpecial'
import PackageCards from '@/components/kashmir/PackageCards'
import ItineraryAccordion from '@/components/kashmir/ItineraryAccordion'
import HotelTierSelector from '@/components/kashmir/HotelTierSelector'
import CostEstimator from '@/components/kashmir/CostEstimator'
import SeasonCalendar from '@/components/kashmir/SeasonCalendar'
import InclusionsExclusions from '@/components/kashmir/InclusionsExclusions'
...
export default function KashmirPage() {
  ...
  const finalCtaWaUrl = buildWhatsAppUrl(
    kashmirData.whatsapp.number,
    kashmirData.whatsapp.finalCtaMessage
  )
  const stickyCtaWaUrl = buildWhatsAppUrl(
    kashmirData.whatsapp.number,
    kashmirData.whatsapp.stickyMessage
  )
  const floaterWaUrl = buildWhatsAppUrl(
    kashmirData.whatsapp.number,
    kashmirData.whatsapp.floaterMessage
  )
  ...
  const jsonLd = {
    ...
    '@graph': [
      ...
      {
        '@type': 'FAQPage',
        'mainEntity': kashmirData.faqs.map((faq) => ({
          '@type': 'Question',
          'name': faq.q,
          'acceptedAnswer': {
            '@type': 'Answer',
      ...
      ...kashmirData.packages.map((pkg) => ({
        ...
        'offers': {
          '@type': 'Offer',
          'priceCurrency': 'INR',
          'price': '19999.00', // Placeholder numeric value for schema requirements
  ...
  return (
    <div data-destination="kashmir" className="min-h-screen bg-white">
      ...
      {/* 3. Trust Strip */}
      <TrustBadgeStrip badges={kashmirData.trustBadges} />

      {/* 4. Quick Summary */}
      <QuickSummaryStrip />

      {/* 5. Why Kashmir Section */}
      <WhyKashmir />
      ...
      {/* 12. Month Guide Calendar */}
      <SeasonCalendar />

      {/* 13. How We Plan Process */}
      <PlanningProcess steps={kashmirData.process} accentColor="#1F6E3F" />
      ...
      <FAQAccordion title="Kashmir Travel FAQs" faqs={kashmirData.faqs}>
        <a
          ...
          className="inline-flex items-center gap-2 text-sm font-bold text-[#1F6E3F] hover:underline"
        >
      ...
      {/* 18. Structured SEO Copy */}
      <SEOContentBlock />

      {/* 19. Other Destinations link */}
      <RelatedDestinations destinations={kashmirData.relatedDestinations} />

      {/* 20. Call-To-Action Foot band */}
      <FinalCTA
        accentColor="#1F6E3F"
      ...
      {/* 22. Mobile Footer CTA Sticky Bar */}
      <StickyMobileCTA
        accentColor="#1F6E3F"
