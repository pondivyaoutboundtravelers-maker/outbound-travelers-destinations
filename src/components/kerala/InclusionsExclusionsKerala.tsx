...
export default function InclusionsExclusionsKerala() {
        ...
        {(['inclusions', 'exclusions'] as const).map((t) => (
          <button
            ...
            className={`flex-1 py-3 text-center font-bold text-sm border-b-2 capitalize transition-all ${
              tab === t ? 'border-[#047857] text-[#047857]' : 'border-transparent text-muted'
            }`}
          >
            {t}
          </button>
      ...
      <div className="hidden md:grid grid-cols-2 gap-8">
        ...
        <div className="bg-white border border-border-soft rounded-2xl p-6 md:p-8 shadow-sm">
          ...
          <ul className="space-y-3.5">
            {keralaData.inclusions.map((item, idx) => {
              const isHouseboat = item.toLowerCase().includes('houseboat')
              return (
                <li key={idx} className="flex items-start gap-3">
                  ...
                  <span className="text-sm text-muted flex-1 leading-normal font-medium">
                    {item}
                    {isHouseboat && (
                      <span className="ml-2 inline-block bg-[#FAF6F0] text-[#047857] text-[9px] font-bold px-2 py-0.5 rounded border border-[#047857]/30 uppercase tracking-wider">
                        All meals included
                      </span>
                    )}
                  </span>
        ...
        <div className="bg-white border border-border-soft rounded-2xl p-6 md:p-8 shadow-sm">
          ...
          <ul className="space-y-3.5">
            {keralaData.exclusions.map((item, idx) => {
              const isWellness = item.toLowerCase().includes('wellness') || item.toLowerCase().includes('ayurveda')
              return (
                <li key={idx} className="flex items-start gap-3">
                  ...
                  <span className="text-sm text-muted leading-normal font-medium">
                    {item}
                    {isWellness && (
                      <span className="ml-2 inline-block text-[9px] font-bold text-[#047857] bg-[#FAF6F0] px-2 py-0.5 rounded border border-[#047857]/20">
                        See Ayurveda section ↑
                      </span>
                    )}
                  </span>
                </li>
                ...
                return (
                  <li key={idx} className="flex items-start gap-2.5">
                    ...
                    <span className="text-xs text-muted flex-1 leading-normal font-medium">
                      {item}
                      {isHouseboat && (
                        <span className="ml-1.5 inline-block bg-[#FAF6F0] text-[#047857] text-[8px] font-bold px-1.5 py-0.5 rounded border border-[#047857]/30">
                          All meals
                        </span>
                      )}
                    </span>
                ...
                return (
                  <li key={idx} className="flex items-start gap-2.5">
                    ...
                    <span className="text-xs text-muted leading-normal font-medium">
                      {item}
                      {isWellness && (
                        <span className="ml-1 inline-block text-[8px] font-bold text-[#047857] bg-[#FAF6F0] px-1.5 py-0.5 rounded border border-[#047857]/20">
                          See Ayurveda ↑
                        </span>
                      )}
                    </span>
