...
export default function GalleryKerala() {
  ...
  const go = (dir: 1 | -1, e: React.MouseEvent) => {
    ...
    setLightboxIdx((prev) => {
      ...
      const next = prev + dir
      if (next < 0) return keralaData.gallery.length - 1
      if (next >= keralaData.gallery.length) return 0
      return next
    })
  }
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowLeft') setLightboxIdx((p) => (p === null ? null : p === 0 ? keralaData.gallery.length - 1 : p - 1))
      if (e.key === 'ArrowRight') setLightboxIdx((p) => (p === null ? null : p === keralaData.gallery.length - 1 ? 0 : p + 1))
    }
    window.addEventListener('keydown', onKey)
    return () => window.removeEventListener('keydown', onKey)
  }, [lightboxIdx])

  // GalleryMosaic: varied-size grid pattern
  // 12 images — pattern: [wide, tall, square, square, wide, tall, square, square, wide, tall, square, square]
  const mosaicSpans = [
    'md:col-span-2 row-span-1', // 0: wide
    'md:col-span-1 md:row-span-2', // 1: tall
    'md:col-span-1 row-span-1',    // 2: square
    'md:col-span-1 row-span-1',    // 3: square
    'md:col-span-1 row-span-1',    // 4: square
    'md:col-span-2 row-span-1',    // 5: wide
    'md:col-span-1 md:row-span-2', // 6: tall
    'md:col-span-1 row-span-1',    // 7: square
    'md:col-span-1 row-span-1',    // 8: square
    'md:col-span-2 row-span-1',    // 9: wide
    'md:col-span-1 row-span-1',    // 10: square
    'md:col-span-1 row-span-1',    // 11: square
  ]
    ...
    <section className="py-16 px-4 md:px-8 max-w-7xl mx-auto">
      <div className="text-center max-w-xl mx-auto mb-12">
        ...
        <p className="text-xs md:text-sm text-muted mt-2">
          Twelve frames — backwaters, hills, culture, wellness, coastline.
        </p>
      </div>

      {/* Mobile: simple 2-col grid */}
      <div className="grid grid-cols-2 gap-3 md:hidden">
        {keralaData.gallery.map((item, idx) => (
          <div
            key={idx}
        ...
        {keralaData.gallery.map((item, idx) => (
          ...
          >
            ...
            <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
              <span className="text-white text-xs font-medium leading-snug">{item.alt.split(' ').slice(0, 5).join(' ')}…</span>
            </div>
          </div>
        ))}
      </div>
      ...
      {lightboxIdx !== null && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/95 p-4" onClick={() => setLightboxIdx(null)}>
          ...
          <div className="relative w-full max-w-4xl h-[70vh] rounded-xl overflow-hidden flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
            <Image src={keralaData.gallery[lightboxIdx].image} alt={keralaData.gallery[lightboxIdx].alt} fill sizes="80vw" className="object-contain" />
            <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 to-transparent text-center">
              <p className="text-white text-xs md:text-sm font-medium">{keralaData.gallery[lightboxIdx].alt}</p>
              <p className="text-white/50 text-[10px] mt-1">{lightboxIdx + 1} / {keralaData.gallery.length}</p>
            </div>
          </div>
