...
export default function KeralaQuickSummary() {
  return (
    <section className="bg-white border-b border-border-soft">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-0">
          {keralaData.quickSummary.map((item, idx) => (
            <div
              key={idx}
              className={`flex items-start gap-4 px-4 ${
                idx < keralaData.quickSummary.length - 1 ? 'lg:border-r lg:border-border-soft' : ''
              }`}
            >
              <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 bg-[#FAF6F0] text-[#047857]">
                {idx === 0 && (
