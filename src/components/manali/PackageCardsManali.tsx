...
export default function PackageCardsManali() {
  const [filteredPackages, setFilteredPackages] = useState<Array<Package>>([])
  const [activeSlide, setActiveSlide] = useState<number>(0)

  useEffect(() => {
    const currentMonth = new Date().getMonth() + 1
    const filtered = manaliData.packages.filter((pkg) => {
  ...
  const handleEnquire = (pkg: Package) => {
    manaliEvents.packageClick(pkg.id, pkg.name)
  ...
  return (
    <section id="manali-packages" className="py-12 md:py-16 px-4 md:px-8 max-w-7xl mx-auto scroll-mt-20">
      ...
      <p className="text-center text-muted mb-10 max-w-xl mx-auto text-sm md:text-base">
        Explore our curated collection of Manali packages, fully customizable to your preferred duration, stays, and activities.
      </p>
          ...
          {filteredPackages.map((pkg) => (
            ...
            >
              ...
              <div className="relative w-full h-[180px] bg-gray-200">
                ...
                {pkg.seasonal && (
                  <span className="absolute top-3 left-3 bg-[#355C7D] text-white text-[9px] font-bold uppercase px-2 py-0.5 rounded tracking-wider z-20">
                    Summer Pack
                  </span>
                )}
              </div>
              ...
              <div className="p-5 flex-1 flex flex-col justify-between">
                <div>
                  <span className="inline-block text-[10px] font-bold text-[#355C7D] bg-[#DEE5EC] px-2.5 py-0.5 rounded-full mb-2">
                  ...
                  <ul className="space-y-1.5 mb-4">
                    {pkg.highlights.map((h, i) => (
                      <li key={i} className="flex items-center gap-2 text-xs text-muted">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#355C7D] flex-shrink-0" />
                ...
                <div className="border-t border-border-soft pt-3 flex justify-between items-center">
                  ...
                  <div className="flex gap-2">
                    <a
                      href={buildWhatsAppUrl(manaliData.whatsapp.number, pkg.whatsappMessage)}
                      target="_blank"
                    ...
                    <button
                      ...
                      className="px-4 py-2 text-white font-bold rounded-lg text-xs bg-[#355C7D] hover:bg-[#355C7D]/90 transition-colors"
                    >
                      Enquire
                    </button>
                  </div>
          ...
          {filteredPackages.map((_, i) => (
            <div
              ...
              style={{
                ...
                backgroundColor: i === activeSlide ? '#355C7D' : '#D1D5DB',
              }}
            />
          ))}
        </div>
        ...
        {filteredPackages.map((pkg) => (
          ...
          >
            ...
            <div className="relative w-full h-[200px] bg-gray-200">
              ...
              {pkg.seasonal && (
                <span className="absolute top-4 left-4 bg-[#355C7D] text-white text-[10px] font-bold uppercase px-2.5 py-0.5 rounded tracking-wider z-20">
                  Summer Special
                </span>
              )}
            </div>
            ...
            <div className="p-6 flex-1 flex flex-col justify-between">
              <div>
                <span className="inline-block text-[10px] font-bold text-[#355C7D] bg-[#DEE5EC] px-2.5 py-0.5 rounded-full mb-3">
                  {pkg.idealFor}
                </span>
                <div className="text-xs text-muted font-medium mb-1">{pkg.duration}</div>
              ...
              <div className="border-t border-border-soft pt-4 flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => handleEnquire(pkg)}
                    className="px-5 h-10 text-white font-bold rounded-lg text-sm bg-[#355C7D] hover:bg-[#355C7D]/90 transition-colors"
                  >
                    Enquire Details
                  </button>
                </div>
