...
export default function TopExperiencesManali() {
  ...
  useEffect(() => {
    ...
    const filtered = manaliData.experiences.filter((exp) => {
  ...
  const handleTileClick = (exp: Experience) => {
    setSelectedExp(exp)
    manaliEvents.galleryView(manaliData.experiences.indexOf(exp))
  }
  ...
  return (
    <section className="py-12 md:py-16 px-4 md:px-8 max-w-7xl mx-auto">
      <h2 className="text-2xl md:text-3xl text-brand-navy font-bold text-center mb-4">
        Top Experiences in Manali
      </h2>
      <p className="text-center text-muted mb-10 max-w-xl mx-auto text-sm md:text-base">
        From deep snow gorges to high rapids and hidden valleys, we weave these unique Manali highlights into your personal plan.
      </p>
        ...
        {filteredExps.map((exp) => (
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-3 md:p-4 z-20 text-white">
              {exp.seasonal && (
                <span className="inline-block bg-[#355C7D] text-[9px] font-bold uppercase px-1.5 py-0.5 rounded mb-1.5 tracking-wider">
                  Seasonal Only
                </span>
              )}
              <h3 className="text-xs md:text-base font-bold font-fraunces leading-tight">
                {exp.title}
              </h3>
            </div>
          </div>
