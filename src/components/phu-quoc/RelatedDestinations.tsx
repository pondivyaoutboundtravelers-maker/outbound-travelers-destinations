...
import { phuQuocRelated } from "@/data/phu-quoc";
...
export default function RelatedDestinations() {
  return (
    <section className="relative bg-neutral-950 px-6 py-16 md:py-24 overflow-hidden border-t border-neutral-900">
      ...
      <div className="absolute top-1/2 left-1/3 w-80 h-80 bg-teal-650/5 rounded-full blur-3xl pointer-events-none bg-teal-650/5" />
          ...
          {phuQuocRelated.map((dest, i) => (
            <Link key={dest.name} href={dest.href}>
              <motion.div
                ...
                whileHover={{ y: -6 }}
                className="group relative h-[240px] rounded-3xl overflow-hidden border border-neutral-800 bg-neutral-900 flex flex-col justify-end p-5 hover:border-teal-500/20 transition-all duration-300 shadow-lg"
              >
                {/* Image */}
                ...
                <div className="relative z-10 flex items-center justify-between">
                  <div className="space-y-0.5">
                    <span className="text-[9px] font-bold uppercase tracking-widest text-[#0D9488]">Destination</span>
                    <h3 className="text-lg font-bold text-white tracking-tight">{dest.name}</h3>
                  </div>
                  <div className="w-8 h-8 rounded-full bg-white/10 group-hover:bg-[#0D9488] text-white flex items-center justify-center transition-colors duration-300">
