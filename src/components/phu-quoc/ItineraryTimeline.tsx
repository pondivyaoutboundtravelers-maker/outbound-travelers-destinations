...
export default function ItineraryTimeline() {
  ...
  const currentDayInfo = phuQuocItinerary.find((item) => item.day === activeDay) || phuQuocItinerary[0];
  ...
  return (
    <section id="itinerary" className="relative bg-neutral-950 px-6 py-16 md:py-24 overflow-hidden border-t border-neutral-900">
      ...
      <div className="absolute top-1/4 right-0 w-96 h-96 bg-teal-650/5 rounded-full blur-3xl bg-teal-650/5" />
      <div className="absolute bottom-1/4 left-0 w-96 h-96 bg-emerald-650/5 rounded-full blur-3xl bg-emerald-650/5" />
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          <p className="text-3xl md:text-4xl lg:text-5xl font-serif font-extrabold text-white">
            Six Days in Tropical Sanctuary
          </p>
          <p className="text-neutral-400 font-light mt-4">
            A slow, romantic, and vetted itinerary combining world-class resorts, sea cable car flights, and private islet yacht cruising.
          </p>
        </motion.div>
            ...
            {phuQuocItinerary.map((item) => (
              <button
                ...
                className={`flex-shrink-0 flex items-center gap-4 text-left p-4 rounded-2xl border transition-all duration-300 w-[180px] lg:w-full ${
                  activeDay === item.day
                    ? "bg-teal-950/20 border-[#0D9488]/50 text-white shadow-[0_0_15px_rgba(13,148,136,0.15)]"
              ...
              >
                <div className={`w-8 h-8 flex items-center justify-center rounded-xl font-bold text-xs ${
                  activeDay === item.day ? "bg-[#0D9488] text-white" : "bg-neutral-850 text-neutral-500"
                }`}>
                  D{item.day}
                </div>
                <div className="space-y-0.5 min-w-0">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-[#0D9488] block">
                    {item.day === 6 ? "Departure" : `Day ${item.day}`}
                  </span>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <div className="space-y-6">
                  {/* Title & Badge */}
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-neutral-805 pb-4 border-b border-neutral-900/50">
                    <div className="space-y-1">
                      <span className="text-xs font-bold text-[#0D9488] tracking-widest uppercase">
                        Timeline Highlight
                      </span>
                      <h3 className="text-2xl font-serif font-extrabold text-white tracking-tight animate-fade-in">
                  ...
                  {/* Description */}
                  <p className="text-neutral-405 text-neutral-400 font-light leading-relaxed text-sm sm:text-base">
                  ...
                  {currentDayInfo.highlights && currentDayInfo.highlights.length > 0 && (
                    <div className="space-y-3">
                      <h4 className="text-xs font-bold uppercase tracking-widest text-neutral-500 font-semibold">
                        Inclusions & Activities
                      </h4>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {currentDayInfo.highlights.map((highlight, idx) => (
                          <div key={idx} className="flex gap-2.5 items-center text-xs text-neutral-300">
                            <div className="p-1 rounded-full bg-teal-950/20 border border-teal-500/20 text-[#0D9488]">
                ...
                <div className="flex justify-between items-center pt-8 border-t border-neutral-850 mt-8 border-t border-neutral-900/50">
                  ...
                  <button
                    disabled={activeDay === phuQuocItinerary.length}
                    onClick={() => setActiveDay((prev) => prev + 1)}
                    className="inline-flex items-center gap-1.5 text-xs uppercase font-bold tracking-wider text-[#0D9488] hover:text-teal-400 disabled:opacity-40 transition-colors"
                  >
                    <span>Next Day</span>
