...
export default function InclusionsExclusionsLadakh() {
  ...
  return (
    <section className="py-12 md:py-16 px-4 md:px-8 max-w-4xl mx-auto">
      ...
      <p className="text-center text-muted mb-8 max-w-xl mx-auto text-sm">
        We maintain complete transparency in our pricing. Here is exactly what you get and what is billed as add-ons.
      </p>
      ...
      <div className="flex border-b border-border-soft mb-8">
        <button
          ...
          style={{
            borderColor: activeTab === 'inclusions' ? '#1F3A8A' : 'transparent',
            color: activeTab === 'inclusions' ? '#1F3A8A' : '#71717A',
          }}
        >
          Trip Inclusions
        </button>
        <button
          ...
          className="flex-1 py-4 text-center font-bold text-sm md:text-base border-b-2 transition-colors focus:outline-none"
          style={{
            borderColor: activeTab === 'exclusions' ? '#1F3A8A' : 'transparent',
            color: activeTab === 'exclusions' ? '#1F3A8A' : '#71717A',
          }}
        >
          Exclusions / Add-ons
        </button>
      ...
      <div className="bg-cream rounded-2xl p-6 md:p-8 mb-8 border border-border-soft">
        {activeTab === 'inclusions' ? (
          <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {ladakhData.inclusions.map((item, idx) => {
              const isOxygen = item.toLowerCase().includes('oxygen')
              return (
          ...
          <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {ladakhData.exclusions.map((item, idx) => (
              <li key={idx} className="flex items-start gap-3">
      ...
      <div className="text-center">
        <button
          onClick={handleCTA}
          className="px-8 h-12 bg-[#1F3A8A] text-white hover:bg-[#1F3A8A]/90 transition-colors font-bold rounded-lg text-sm shadow-sm"
        >
