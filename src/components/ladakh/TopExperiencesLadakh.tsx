...
export default function TopExperiencesLadakh() {
  const [filteredExps, setFilteredExps] = useState<Experience[]>([])
  const [selectedExp, setSelectedExp] = useState<Experience | null>(null)

  useEffect(() => {
    const currentMonth = new Date().getMonth() + 1
    const filtered = ladakhData.experiences.filter((exp) => {
  ...
  const handleTileClick = (exp: Experience) => {
    setSelectedExp(exp)
    ladakhEvents.galleryView(ladakhData.experiences.indexOf(exp))
  }
  ...
  return (
    <section className="py-12 md:py-16 px-4 md:px-8 max-w-7xl mx-auto">
      ...
      <p className="text-center text-muted mb-10 max-w-xl mx-auto text-sm md:text-base">
        From the surreal blue shores of Pangong Lake to the world&apos;s highest motorable road — here are the experiences we craft into your itinerary.
      </p>
        ...
        {filteredExps.map((exp) => (
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-3 md:p-4 z-20 text-white">
              {exp.seasonal && (
                <span className="inline-block bg-[#1F3A8A] text-[9px] font-bold uppercase px-1.5 py-0.5 rounded mb-1 tracking-wider">
                  Seasonal Special
                </span>
              )}
