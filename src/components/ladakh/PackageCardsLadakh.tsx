...
export default function PackageCardsLadakh() {
  const [filteredPackages, setFilteredPackages] = useState<Package[]>([])
  const [activeSlide, setActiveSlide] = useState(0)

  useEffect(() => {
    const currentMonth = new Date().getMonth() + 1
    const filtered = ladakhData.packages.filter((pkg) => {
  ...
  const handleEnquire = (pkg: Package) => {
    ladakhEvents.packageClick(pkg.id, pkg.name)
  ...
  return (
    <section id="ladakh-packages" className="py-12 md:py-16 px-4 md:px-8 max-w-7xl mx-auto scroll-mt-20">
      ...
      <p className="text-center text-muted mb-10 max-w-xl mx-auto text-sm md:text-base">
        Every package is a starting point — customize hotels, extend days, add bike hire or trekking. Fully flexible.
      </p>
      ...
      <div className="md:hidden">
        ...
        >
          {filteredPackages.map((pkg) => (
            <div
              key={pkg.id}
              className="snap-start min-w-[85vw] flex-shrink-0 relative h-[420px] rounded-xl overflow-hidden shadow-md"
            >
              ...
              <div className="absolute inset-0 p-5 z-20 flex flex-col justify-between text-white">
                <div>
                  <span className="text-[9px] font-bold uppercase px-2 py-0.5 rounded tracking-wider text-white bg-[#1F3A8A]">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex justify-between items-center mt-3 pt-3 border-t border-white/20">
                    ...
                    <div className="flex gap-2">
                      <a
                        href={buildWhatsAppUrl(ladakhData.whatsapp.number, pkg.whatsappMessage)}
                        target="_blank"
                      ...
                      <button
                        ...
                        className="px-4 py-2 text-white font-bold rounded-lg text-xs bg-[#1F3A8A] hover:bg-[#1F3A8A]/90 transition-colors"
                      >
                        Enquire
                      </button>
                    </div>
          ...
          {filteredPackages.map((_, i) => (
            <div
              ...
              style={{
                ...
                backgroundColor: i === activeSlide ? '#1F3A8A' : '#D1D5DB',
              }}
            />
          ))}
        </div>
      ...
      <div className="hidden md:grid grid-cols-3 gap-6">
        {filteredPackages.map((pkg) => (
          <div
            key={pkg.id}
            className="relative h-[420px] rounded-xl overflow-hidden shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300"
          >
            <div className="absolute inset-0 z-0 bg-gray-200">
            ...
            <div className="absolute inset-0 p-6 z-20 flex flex-col justify-between text-white">
              ...
              <div className="flex flex-col gap-2">
                <span className="text-xs text-white/80 font-medium">{pkg.duration}</span>
                <h3 className="text-xl md:text-2xl font-fraunces font-bold leading-tight">{pkg.name}</h3>
                <div className="flex flex-wrap gap-1.5 mt-1">
                  {pkg.highlights.map((h, i) => (
                ...
                <div className="flex justify-between items-center mt-4 pt-4 border-t border-white/20">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => handleEnquire(pkg)}
                      className="px-5 h-10 text-white font-bold rounded-lg text-sm bg-[#1F3A8A] hover:bg-[#1F3A8A]/90 transition-colors"
                    >
                      Enquire Details
                    </button>
                  </div>
