...
export default function SeasonCalendarBali() {
  ...
  const handleMonthClick = (idx: number, month: string) => {
    ...
    baliEvents.seasonFilterClick(month)
  }
  ...
  return (
    <section className="py-12 md:py-16 px-4 md:px-8 max-w-7xl mx-auto">
      ...
      <p className="text-center text-muted mb-8 max-w-xl mx-auto text-sm md:text-base">
        Bali is a year-round destination, but weather patterns do shift. Tap a month below to see details, then browse the seasonal cards.
      </p>
          ...
          {baliData.months.map((m, idx) => (
            <button
              ...
              className={`px-4 py-2 border rounded-full text-sm font-semibold transition-all focus:outline-none ${getPillStyles(
                m.rating
              )} ${activeMonthIdx === idx ? 'ring-2 ring-[#D97706] scale-105' : 'scale-100'}`}
            >
              {m.month}
            </button>
          ))}
      ...
      {activeMonthIdx !== null && (
        <div className="max-w-xl mx-auto mb-12 bg-[#FEF3C7] text-brand-navy border border-[#D97706]/30 p-4 rounded-xl text-center text-sm font-medium animate-fade-in relative">
          <button
        ...
        {baliData.seasons.map((s) => (
          ...
          >
            ...
            <div className="p-4 md:p-5 flex-1 flex flex-col justify-between">
              <div>
                <span className="block text-[10px] text-[#D97706] uppercase tracking-wider font-bold mb-1">
                  {s.months}
                </span>
