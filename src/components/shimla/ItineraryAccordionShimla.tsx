...
export default function ItineraryAccordionShimla() {
  ...
  useEffect(() => {
    const handleHash = () => {
      ...
      if (hash && hash.startsWith('#day-')) {
        ...
        if (!isNaN(parsed) && parsed >= 1 && parsed <= shimlaData.itinerary.length) {
          setOpenDay(parsed)
        }
      }
    }
  ...
  const handleToggle = (day: number) => {
    ...
    if (day !== 0) shimlaEvents.itineraryExpand(day)
  }
  ...
  return (
    <section className="py-12 md:py-16 px-4 md:px-8 max-w-4xl mx-auto">
      <h2 className="text-2xl md:text-3xl text-brand-navy font-bold text-center mb-8">
        Sample Shimla Itinerary — 5 Nights, 6 Days
      </h2>

      <div className="space-y-4 mb-10">
        {shimlaData.itinerary.map((dayItem) => {
            ...
            >
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-[#2F5233] text-white flex items-center justify-center font-bold text-sm flex-shrink-0">
                    D{dayItem.day}
                  </div>
                  <div>
                ...
                <span
                  className="transform transition-transform text-[#2F5233] flex-shrink-0"
              ...
              >
                <div className="p-5 border-t border-border-soft bg-cream">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 mt-2">
                    ...
                    <span className="text-xs font-bold text-[#2F5233] bg-[#E8EFE6] px-3 py-1 rounded-full uppercase tracking-wider">
                      Stay: {dayItem.stay}
                    </span>
                  </div>
                </div>
      ...
      <div className="text-center">
        <button
          ...
          className="inline-flex items-center gap-2 px-6 h-12 bg-transparent border-2 border-[#2F5233] text-[#2F5233] hover:bg-[#E8EFE6] transition-colors font-bold rounded-lg text-sm"
        >
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 p-4">
          ...
          <div className="relative bg-white rounded-2xl p-6 max-w-sm w-full z-10 shadow-2xl">
            ...
            <p className="text-xs text-muted mb-4 leading-normal">
              Share your details to receive the complete 6-day Shimla itinerary with pricing, toy train timings, and hotel choices.
            </p>
            <form onSubmit={handleDownloadPDFSubmit} className="space-y-3">
              <input
                ...
                className="w-full px-3 py-2 border border-border-soft rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#2F5233]/30"
              />
              <input
                type="tel"
                required
