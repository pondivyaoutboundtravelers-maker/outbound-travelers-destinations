...
export default function GalleryGridShimla() {
  const [lightboxIndex, setLightboxIndex] = useState<number | null>(null)

  const handleOpenLightbox = (index: number) => {
    setLightboxIndex(index)
    shimlaEvents.galleryView(index)
  }

  const handlePrev = (e: React.MouseEvent) => {
    e.stopPropagation()
    setLightboxIndex((prev) => (prev === null || prev === 0 ? shimlaData.gallery.length - 1 : prev - 1))
  }

  const handleNext = (e: React.MouseEvent) => {
    e.stopPropagation()
    setLightboxIndex((prev) => (prev === null || prev === shimlaData.gallery.length - 1 ? 0 : prev + 1))
  }
  ...
  return (
    <section className="py-12 md:py-16 px-4 md:px-8 max-w-7xl mx-auto">
      ...
      <p className="text-center text-muted mb-10 max-w-xl mx-auto text-sm md:text-base">
        View real scenery of Shimla valleys, historic buildings, Kufri snow slopes, and boutique heritage stays.
      </p>

      {/* Grid */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {shimlaData.gallery.map((img, idx) => (
          <div
            key={idx}
        ...
        <div className="fixed inset-0 z-[120] bg-black/90 flex items-center justify-center p-4">
          ...
          <div className="relative max-w-4xl max-h-[80vh] w-full h-full flex flex-col items-center justify-center z-20">
            <div className="relative w-full h-full max-h-[70vh]">
              <Image
                src={shimlaData.gallery[lightboxIndex].image}
                alt={shimlaData.gallery[lightboxIndex].alt}
                fill
            ...
            <p className="text-white text-center mt-4 text-xs md:text-sm font-semibold tracking-wide">
              {shimlaData.gallery[lightboxIndex].alt}
            </p>
          </div>
        </div>
      )}
