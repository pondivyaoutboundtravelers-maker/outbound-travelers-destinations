...
const BASE = { destination_id: 'amritsar', destination_type: 'domestic' as const }

export const amritsarEvents = {
  ...
  seasonFilterClick: (month: string) => trackEvent({ ...BASE, event_name: 'amritsar_season_filter_click', month }),
  ...
  // Amritsar-specific events
  comboAddonClicked: (comboLabel: string) => trackEvent({ ...BASE, event_name: 'amritsar_combo_addon_clicked', combo_label: comboLabel }),
  wagahInfoViewed: () => trackEvent({ ...BASE, event_name: 'amritsar_wagah_info_viewed' }),
  langarChipClicked: () => trackEvent({ ...BASE, event_name: 'amritsar_langar_chip_clicked' })
}
