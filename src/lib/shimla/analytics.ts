...
const BASE = { destination_id: 'shimla', destination_type: 'domestic' as const }

export const shimlaEvents = {
  ...
  heritagePropertyView: () => trackEvent({ ...BASE, event_name: 'heritage_property_tier_view' }),
  ...
  seasonFilterClick: (month: string) => trackEvent({ ...BASE, event_name: 'shimla_season_filter_click', month }),
  toyTrainCtaClick: () => trackEvent({ ...BASE, event_name: 'toy_train_cta_click' }),
  snowSeasonCtaClick: () => trackEvent({ ...BASE, event_name: 'snow_season_cta_click' }),
